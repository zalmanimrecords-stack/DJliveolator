...
/// with <paramref name="Peaks"/>; empty when not computed. The strip draws this band in FRONT, bright,
/// so a DJ can align downbeats by eye for sync (doc 22 — deck waveform).</param>
/// <param name="MidPeaks">The mid band (vocals/synths body) magnitude per bucket, aligned 1:1 with
/// <paramref name="Peaks"/>; empty when not computed. Drawn as the waveform body behind the kick.</param>
/// <param name="HighPeaks">The high band (hats/air) magnitude per bucket, aligned 1:1 with
/// <paramref name="Peaks"/>; empty when not computed. Drawn as pale caps behind the body.</param>
public sealed record WaveformOverview(
    IReadOnlyList<float> Peaks,
    double DurationSeconds = 0,
    IReadOnlyList<float>? LowPeaks = null,
    IReadOnlyList<float>? MidPeaks = null,
    IReadOnlyList<float>? HighPeaks = null)
{
    ...
    /// <summary>True when the full low/mid/high band set was computed (the layered strip needs all
    /// three; a low band alone still renders as the kick overlay over the broadband body).</summary>
    public bool HasBands => HasLowBand && MidPeaks is { Count: > 0 } && HighPeaks is { Count: > 0 };
