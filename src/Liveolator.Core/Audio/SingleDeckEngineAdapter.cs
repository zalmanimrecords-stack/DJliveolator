...
internal sealed class SingleDeckEngineAdapter : IMultiDeckPlaybackEngine
{
    ...
    // The legacy single-deck engine has no rate/tempo surface, so a pitch-bend is a no-op.
    public void PitchBend(int slot, double bendFraction) => EnsureSlot(slot);
    ...
    public void ClearHotCue(int slot, int cueIndex) => EnsureSlot(slot);

    // The legacy single-deck engine has no hot-cue memory or persistence, so there is nothing to reload.
    public void ReloadHotCues(int slot) => EnsureSlot(slot);
