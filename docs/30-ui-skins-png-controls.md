...
## Themes can style controls + carry a background image (capability)
...
- **Per-control colour tokens** — `KnobArcColor / KnobTrackColor / KnobCapColor / KnobPointerColor` and
  ...
  resource the Knob/Fader styles bind to, so a theme can recolour the knobs/faders without disturbing the
  surface/text tokens. An active control skin still wins (applied after the theme). The built-in `Spartan`
  theme defines all of these at their defaults so switching back to it resets the controls cleanly.
...
> The `Analog` (chrome+wood) theme that first exercised this capability was **removed** (2026-06-11, owner
> rejected the look). The capability above remains, ready for a future textured theme; no built-in theme
> ships a `BackgroundImage` today.
...
> the base under it — a fully custom cap colour needs a photographic `SkinnableKnob` filmstrip. The arc
> colour and the fader thumb are fully themeable.
