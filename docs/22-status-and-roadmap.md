...
- **DJ engine — 4 of 5 doc-20 critical gaps moved forward; two fully closed:**
  ...
  - 🟡 **Beatgrid first-beat anchor + phase-sync** — built (`FirstBeatEstimator`, real `PhaseAlignmentCalculator`) but **not threaded to the engine** (see §3, keystone).
...
|---|------|--------|----------------|-----------|
| **A1** | **Thread the first-beat anchor to the engine** (keystone). After each `engine.Load`, call `SetDeckFirstBeat(slot, track.Bpm?.FirstBeatSeconds ?? 0)` — via the composition root or a new `DeckSetFirstBeat` action / parsed load field. Add a test asserting `PhaseAlignToLeader` uses a non-zero anchor. Add `IsManual` grid protection. | S | Makes phase-sync actually align beats → the headline "one-button sync." Unblocks musical loops & on-grid cues & beat-synced visuals. | `DeckActionHandler.LoadTrack`, `LibrariesViewModel` 334/351, `TrackContextActions`, `TwoDeckBassEngine.PhaseAlignToLeader` |
