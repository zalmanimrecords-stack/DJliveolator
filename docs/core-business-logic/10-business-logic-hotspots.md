...
- **Source of truth:** file sizes and call sites at commit `b809ec7`.
...
| `src/Liveolator.App/Features/Libraries/LibrariesViewModel.cs` | 1928 lines | The largest single file in the repository. Scan, search, facets, badges, enrichment, import, repair, deck loading and playlist building all converge in one presentation type | Library management becomes unmaintainable; repair and relocation are among the destructive paths |
...
| `src/Liveolator.Core/Library/Music/MusicLibrary.cs` + `Analysis/TrackAnalyzer.cs` | 664 + 191 lines (was 486 + 150) | An expensive partial-failure workflow combining filesystem identity, decoding, metadata, analysis provenance and persistence | A scan that corrupts or loses catalog state costs the user hours of re-analysis |
...
| `src/Liveolator.Core/Studio/Set` | 1813 lines across 15 files; `DjSetArranger` 490, `SetTransitionPlanner` 442 | The newest concentration in the codebase and the densest arithmetic in it: candidate gating, tempo policy (fixed, median or travelling), phrase-quantized anchor placement, warp measurement, loudness matching and join auditing. Every rule is numeric and every failure is audible rather than thrown | A set that renders silence, flams on the kick, or drops records without saying why — and the only consumer is an agent that cannot hear the result |
...
`Liveolator.Core.Tests` is large (203 files) and the pure rules are well covered. The gaps are at the
...
`MusicLibrary` grew 37% (486 → 664 lines) in this period while remaining the owner of the scan,
identity, manual-analysis-protection and persistence rules — it is the fastest-growing hotspot in
the set and the one most likely to need splitting next ([15](./15-refactor-recommendations.md)).
